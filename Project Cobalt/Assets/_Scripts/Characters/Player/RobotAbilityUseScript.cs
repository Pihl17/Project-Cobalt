...
public class RobotAbilityUseScript : RobotCharacterScript
{
	...
	protected AbilityContext GetAbilityContext(InputActionPhase phase) {
		...
		return new AbilityContext(phase, transform, lockOnTarget != null ? lockOnTarget.position - transform.position : transform.forward * lockOnDistance, this);
		/*if (lockOnTarget)
